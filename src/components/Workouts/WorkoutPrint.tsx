...
  useEffect(() => {
    if (user) { // Adicione esta verificação para garantir que o usuário existe
      loadPersonalInfo();
    }
  }, [user]);
...
    try {
      // Remova o .single() e pegue o resultado como um array
...
      if (data && data.length > 0) {
        // Se houver dados, pegue o primeiro item do array
        personalData = data[0];
      } else {
        // Se não houver dados, crie um registro padrão
...
          })
          .select()
          .single(); // O .single() aqui é seguro, pois estamos inserindo um único item
